...
test.describe('a control on a screen full of them', () => {
     ...
     *
     * Held back, and not because the fix is wrong. Both pages name their
     * control for the thing it opens, and the dev server renders exactly
     * that: `aria-label="Launcher nasıl kullanılır"`. A production build does
     * not, and 2026-09-21 was spent failing to find out why. What was ruled
     * out, so the next person does not repeat it:
     *
     *   - the built chunk carries the code. `"aria-label":f("readGuideFor",
     *     {title:a.title})` is in `.next-prod/server/chunks/ssr`.
     *   - the message is in the catalogue the server reads. Redis holds
     *     `downloads.readGuideFor = "How to use {title}"`.
     *   - it is not a stale build: `.next-prod` was deleted and rebuilt.
     *   - it is not a stale cache: the catalogue was dropped and the server
     *     restarted afterwards. There is no `cacheHandler`, and the only
     *     Redis keys are the two catalogues.
     *   - it is not the registry: `module-page-registry.tsx` imports the file
     *     that was edited, and was regenerated by the same build.
     *   - it is not one locale, one module, or one key: `/en` and `/tr`,
     *     `changelog` and `downloads`, both new keys.
     *
     * So the element renders and the attribute does not, which means the
     * translator returned nothing for a message that is there. A module's
     * *client* component change does reach a production build - the popup's
     * buttons in the same release did. Turning these on before that is
     * understood would make CI red for a reason that is not this product's
     * behaviour.
     */
    for (const [what, path] of [
        ...
        test.fixme(`says which one it is, for ${what}`, async ({ page }) => {
